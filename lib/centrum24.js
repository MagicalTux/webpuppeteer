...
//			sys.log("Found account number ["+data.account+"] of type "+data.account_type+" with balance "+data.balance);
	...
	this.scanAccountByNumber = function(n, callback, doskip) {
		for(var i = 0; i < this.accounts.length; i++) {
			if (this.accounts[i].account == n)
				return this.scanAccountHistory(i, callback, doskip);
		}
		return false;
	};

	this.scanAccountHistory = function(i, callback, doskip) {
		...
		while(true) {
			if (page_cnt > 100) break; // max 100 pages
