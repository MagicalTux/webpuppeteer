...
centrum24 = function() {
	...
	// in english (new english selector 2014-01-18)
//	this.tab.document().findAllContaining("English")[0].click();
	this.tab.document().findFirst("div#lang a").click();
	this.tab.wait();
...
//			sys.log("Found account number ["+data.account+"] of type "+data.account_type+" with balance "+data.balance);
	...
	this.processSwift = function(data, sms_callback) {
		...
		var sms = sms_callback(data);

		if (sms) {
			try {
				this.tab.document().getElementById("authorization_response").setAttribute("value", sms);
				this.tab.document().getElementsByName("smsOrTokenInput:confirm")[0].eval("this.click()");
				this.tab.waitFinish(500);
			} catch(e) {
				sys.log("got error "+e);
