...
//			sys.log("Found account number ["+data.account+"] of type "+data.account_type+" with balance "+data.balance);
	...
	this.scanAccountHistory = function(i, callback, doskip, datelimit) {
		...
		if (datelimit) {
			...
			this.tab.document().findFirst("input[type=radio][value=radio1][name=\"filterGroupContainer:filterGroup\"]").eval("this.checked='true'");
	...
	this.processSwift = function(data) {
		...
		this.tab.document().getElementsByName('recipientInputPanel:streetBorder:street')[0].setAttribute("value", data["meta"]["address"].replace(/°/g,"o").replace(/º/g,"o").substr(0,30));
		...
		try {
			...
			this.tab.document().getElementsByName("buttonC:btnConfirm")[0].eval("this.click()");
			this.tab.waitFinish(50);
		} catch(e) {
			sys.log("got error "+e);
		}
