...
jpnet = function() {
	...
	this.getTodayDeposits = function() {
		...
		return this.getDepositsCallback(function() { this.tab.document().findFirst("form").eval("ToHonjitsu(this)"); this.tab.waitFinish(500); });
	};

	this.getYesterdayDeposits = function() {
		sys.log("JapanNet: Getting history for yesterday...");
		return this.getDepositsCallback(function() { this.tab.document().findFirst("form").eval("ToZenjitsu(this)"); this.tab.waitFinish(500); });
	};
	...
	this.getDayDeposits = function(d_year, d_month, d_day) {
		...
		var cb = function() {
			...
			this.tab.document().findFirst("form").eval("ToKikan(this)");
			this.tab.waitFinish(500);
		};
