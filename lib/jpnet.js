...
jpnet = function() {
	...
	this.getTodayDeposits = function() {
		...
		return this.getDepositsCallback(function(tab) { tab.document().findFirst("form").eval("ToHonjitsu(this)"); });
	};

	this.getYesterdayDeposits = function() {
		sys.log("JapanNet: Getting history for yesterday...");
		return this.getDepositsCallback(function(tab) { tab.document().findFirst("form").eval("ToZenjitsu(this)"); });
	};
