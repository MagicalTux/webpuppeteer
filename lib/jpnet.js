...
jpnet = function() {
	...
	this.getTodayDeposits = function() {
		...
		return this.getDepositsCallback(function(tab) { tab.document().findFirst("form").eval("ToHonjitsu(this)"); tab.waitFinish(500); });
	};

	this.getYesterdayDeposits = function() {
		sys.log("JapanNet: Getting history for yesterday...");
		return this.getDepositsCallback(function(tab) { tab.document().findFirst("form").eval("ToZenjitsu(this)"); tab.waitFinish(500); });
	};
	...
	this.getDepositsCallback = function(cb) {
		try {
			...
			cb(this.tab); // this will select the right period
